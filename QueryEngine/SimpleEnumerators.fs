...
module SimpleEnumerators =
            ...
            seq {
                if phraseParts.Length > 0 then
                    while not <| phraseParts.[0].AtEnd do
                        ...
                        let foldFunc (expectedDocID, expectedSeqID, expectedSecID, expectedPosition, expectedPID, lineUp) (phrasePartEnumerator : TokenPostingList) =
                            let lineUp' = 
                                lineUp &&
                                (phrasePartEnumerator.Current.DocumentID = expectedDocID) &&
                                (phrasePartEnumerator.Current.SequenceID = expectedSeqID) &&
                                (phrasePartEnumerator.Current.SectionID = expectedSecID) &&
                                (phrasePartEnumerator.Current.DocumentPosition = expectedPosition) &&
                                (phrasePartEnumerator.Current.ParagraphID = expectedPID)
                            (expectedDocID, expectedSeqID, expectedSecID, expectedPosition + 1, expectedPID, lineUp')

                        let initialState =
                            (firstTokenPosition.DocumentID, 
                             firstTokenPosition.SequenceID, 
                             firstTokenPosition.SectionID,
    ...
    let createExactPhraseEnumeratorEx(phraseParts : TokenPostingList[]) =
        
        // use Linq to generate same results

        // "minds of the mortals"
        // {{minds,4,0:0.1,0,2,2,7}}
        // {{minds,4,1:3.6,1,32,67,409}}
        // {{minds,4,130:4.5,130,43,28,189}}

        // Get sequence of phraseParts tokens to rebuid enire phrase using text positions
        // tokPos0 = {{minds,4,0:0.1,0,2,2,7}}      tp0
        // tokPos1 = {{of,5,0:0.1,0,2,3,13}}        tp1 = t0 + (t0 - 1) -> next position [tpo + (tpo - 1)] * text position aka tpoId
        // tokPos2 = {{the,3,0:0.1,0,2,4,16}}       tp2 = t1 + (t1 - 1) -> next position
        // tokPos2 = {{mortals,6,0:0.1,0,2,5,20}}   tp3 = tpo + (tpo - 1) -> next position

        let fst = phraseParts[0].BasePostingList
        let snd = phraseParts[1].BasePostingList
        let thr = phraseParts[2].BasePostingList
        let fth = phraseParts[3].BasePostingList

        let fstDocId = fst.Head.ElementAt(0).DocumentID
        let fstSeqId = fst.Head.ElementAt(0).SequenceID
        let fstSecId = fst.Head.ElementAt(0).SectionID
        let fstDocPos = fst.Head.ElementAt(0).DocumentPosition
        let fstTrmPos : int = fst.Head.ElementAt(0).TermPosition
        let fstToken = fst.Head.ElementAt(0).Token

        let indexOffset = fstTrmPos - 1
        let nxtTrmPos : int = fstTrmPos + indexOffset
        let nxtDocPos = fstDocPos + 1
        let nxtNxtDocPos = nxtDocPos + 1

        let occ0 = fst.Head.ElementAt(0)
        let occ1 = fst.Head.ElementAt(1)
        let occ2 = fst.Head.ElementAt(2)
        let occ3 = fst.Head.ElementAt(3)

        let tokPos0 = fst.Head.Where(fun o -> o.DocumentID = fstDocId && 
                                              o.SequenceID = fstSeqId).FirstOrDefault()

        let tokPos1 = snd.Head.Where(fun o -> o.DocumentID = fstDocId && 
                                              o.SequenceID = fstSeqId && 
                                              o.DocumentPosition = nxtDocPos).FirstOrDefault()

        let tokPos2 = thr.Head.Where(fun o -> o.DocumentID = fstDocId && 
                                              o.SequenceID = fstSeqId && 
                                              o.DocumentPosition = (nxtDocPos + 1)).FirstOrDefault()

        let tokPos3 = fth.Head.Where(fun o -> o.DocumentID = fstDocId && 
                                              o.SequenceID = fstSeqId && 
                                              o.DocumentPosition = (nxtDocPos + 2)).FirstOrDefault()
    ...
    let conjunctiveQueryWithRangeSEC (iter1 : TokenPostingList) (iter2 : TokenPostingList) =
        ...
        let query1 = query { for tp in seq1 do
                             let docId = tp.DocumentID
                             let secId = tp.SectionID
                             select (docId, secId) }
        let query2 = query { for tp in seq2 do 
                             let docId = tp.DocumentID
                             let secId = tp.SectionID
