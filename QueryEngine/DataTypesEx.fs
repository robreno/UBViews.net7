...
type ExtendedToken =
    { TokenID: int;
      ...
      Lexeme: string }
    override this.ToString() =
        sprintf "%d| %d| %d| %d| %s| %s| %d| %s" 
            this.TokenID this.DocumentID this.SequenceID this.DocumentPosition this.PositionID this.ParagraphId this.TermPosition this.Lexeme
...
type TokenPositionEx =
    ...
    interface IComparable with
        member this.CompareTo((rhs : obj)) =
            let rhsAsTokenPosition = rhs :?> TokenPositionEx
            let thisAsIComparable = this :> IComparable<TokenPositionEx>
            thisAsIComparable.CompareTo(rhsAsTokenPosition)

    interface IComparable<TokenPositionEx> with
        member this.CompareTo((rhs : TokenPositionEx)) =
            let lhs = this
    ...
    override this.Equals(o) =
        let thisAsIC = this :> IComparable<TokenPositionEx>
        match o with
        | null -> false
        | :? TokenPositionEx as o -> (thisAsIC.CompareTo(o) = 0)
        | _ -> false

/// Posting list for a given token. Note that the token occurences are
/// sorted.
/// Most Current Used For Data
type TokenPostingListEx = 
    { PostingID : PostingListID; Token : RawToken; TokenOccurrences : seq<TokenPositionEx> }
...
/// Most Current Used For Data
let makeTokenPositionEx (token: RawToken)
                        ...
                        (sectionID: SectionID)
                        (documentPosition: DocumentPosition) 
                        (termPosition: TermPosition) : TokenPositionEx =
    { Token = token;
      PostingListID = postingListID;
      ParagraphID = paragraphID;
      DocumentID = documentID; 
